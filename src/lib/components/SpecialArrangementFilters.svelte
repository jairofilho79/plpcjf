<script>
  ...
  $: {
    allChecked = selected.length === available.length && available.length > 0;
  ...
  function handleTodosClick() {
    if (allChecked) {
      ...
      dispatch('selectAll', { items: available });
    }
  }
  
  /**
   * @param {string} specialArrangement
   */
  function handleSpecialArrangementClick(specialArrangement) {
    dispatch('toggle', { item: specialArrangement });
  }
  
  /**
   * @param {string} specialArrangement
   */
  function handleSpecialArrangementLongPress(specialArrangement) {
    dispatch('selectOnly', { item: specialArrangement });
  }
</script>
...
{#if available.length > 0}
  <div class="w-full max-w-4xl p-4 bg-card-color rounded-lg border-2 flex flex-wrap gap-2 items-center justify-center filter-container">
    ...
    {#each available as specialArrangement}
      {@const isSpecialArrangementActive = selected.includes(specialArrangement)}
      <GestureButton
        on:click={() => handleSpecialArrangementClick(specialArrangement)}
